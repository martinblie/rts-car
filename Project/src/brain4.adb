...
package body Brain4 is
   ...
   task body ObstacleAvoider is
      ...
      servoInterval : Time_Span := Milliseconds(200);
      servo1Start : Time;
      servo2Start : Time;
      servo3Start : Time;
   begin
      ...
      loop
         ...
         if Clock > servo1Start + servoInterval then
            turnServo(dir45R);
            servo1Start := servo1Start + 4*servoInterval;
         end if;
         
         if Clock > servo2Start + servoInterval then
            turnServo(dirF);
            servo2Start := servo2Start + 2*servoInterval;
            end if;
         if Clock > servo3Start + servoInterval then
            turnServo(dir45L);
            servo3Start := servo3Start + 4*servoInterval;
         end if;
